...
class APIManager: SessionManager {
    ...
    static let consumerKey = "LaghwVCyTK8Kgt9Us1d56xVqT"
    static let consumerSecret = "IEOquAIy6nFRbC4MKAdqpqO10ZF3k0UH8C7VXJTTPpVw1ZYy85"
        ...
        oauthManager.authorize(withCallbackURL: callbackURL, success: { (credential, _response, parameters) in
            ...
            self.getCurrentAccount(completion: { (user, error) in
                if let error = error {
                    ...
                    print("Screen Name \(String(describing: user.screenName))")
                    print("Image: \(String(describing: user.profile_image_url_https))")
    ...
    func retweet(_ tweet: Tweet, completion: @escaping (Tweet?, Error?) -> ()) {
        ...
        let parameters = ["id": tweet.id]
        let urlString = "https://api.twitter.com/1.1/statuses/retweet/" + String(describing: tweet.id) + ".json"
        request(urlString, method: .post, parameters: parameters, encoding: URLEncoding.queryString).validate().responseJSON { (response) in
            if response.result.isSuccess,
                let tweetDictionary = response.result.value as? [String: Any] {
                let tweet = Tweet(dictionary: tweetDictionary)
    ...
    func unretweet(_ tweet: Tweet, completion: @escaping (Tweet?, Error?) -> ()) {
        let parameters = ["id": tweet.id]
        let urlString = "https://api.twitter.com/1.1/statuses/unretweet/" + String(describing: tweet.id) + ".json"
        request(urlString, method: .post, parameters: parameters, encoding: URLEncoding.queryString).validate().responseJSON { (response) in
            if response.result.isSuccess,
                let tweetDictionary = response.result.value as? [String: Any] {
                let tweet = Tweet(dictionary: tweetDictionary)
